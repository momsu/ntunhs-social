      ...
      <div class="w3-row">
         ...
         <div class="w3-col m9">
               <div class="w3-row-padding">
                  ...
                  <?php
                  // 抓取由本使用者發布的照片
                  $query = 'SELECT * FROM `posts` JOIN `users` ON `posts`.`ID`=`users`.`ID` WHERE `posts`.`ID`="' . $_COOKIE['id'] . '"ORDER BY `pID` DESC';
                  $result = $link->query($query);
                  foreach ($result as $row) {
                     if ($row['pPic'] != 'NULL') {
                        echo '<div class="w3-card w3-white w3-round w3-margin-left w3-margin-right w3-margin-bottom" 
                        style="display: inline-block; width: 300px;">
                           <img src="' . $row['pPic'] . '" style="display: inline-block; width:100%; padding:10px" alt="img">
                        </div>';
                     }
                  }
                  ?>
